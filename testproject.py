...
def test_age(client):
    responses.add(
        responses.GET,
        "https://api.agify.io",
        json={"age": 44, "count":1049384,"name":"Anthony"},
        status=200
    )
    client.post("/regiister", data={"email": "test@test.com","password": "testpassword"})
    client.post("/login",data={"email": "test@test.com","password": "testpassword"})

    response = client.post("/age",data={"name":"Anthony"})

    assert b"You are 20 years old" in response.data
